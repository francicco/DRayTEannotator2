...
def normalize_structure_type(value: str | None) -> str | None:
    ...
    if v in {"TSD", "TARGET_SITE_DUPLICATION"}:
        return "TSD"
    if v in {"POLYA", "POLY_A", "POLYA_TAIL", "SINE"}:
        return "POLYA"
...
def summarize_structure_evidence(
    ...
    for ev in evidence:
        ...
        fam = summary.setdefault(ev.family_id, _empty_summary())

        if ev_type == "LTR":
            fam["ltr_present"] = True
            fam["ltr_structural_type"] = ev.ltr_structural_type
            fam["ltr_score"] = max(fam["ltr_score"], ev.ltr_score)
            fam["ltr_confidence"] = ev.ltr_confidence
            fam["tg_ca_motif"] = fam["tg_ca_motif"] or ev.tg_ca_motif
            fam["ppt_like"] = fam["ppt_like"] or ev.ppt_like
            ...
            fam["polyA_tail_len"] = max(fam["polyA_tail_len"], ev.polyA_tail_len)
            fam["polyA_fraction"] = max(fam["polyA_fraction"], ev.polyA_fraction)
            fam["poliii_a_box"] = fam["poliii_a_box"] or ev.poliii_a_box
            fam["poliii_b_box"] = fam["poliii_b_box"] or ev.poliii_b_box
            fam["poliii_score"] = max(fam["poliii_score"], ev.poliii_score)
            fam["sine_score"] = max(fam["sine_score"], ev.sine_score)
            fam["sine_candidate"] = fam["sine_candidate"] or ev.sine_candidate
...
def load_structure_evidence_tsv(path) -> list[StructureEvidence]:
    ...
    with open(path) as fh:
        ...
        if reader.fieldnames and "evidence_type" in reader.fieldnames:
            for row in reader:
                evidence_type = normalize_structure_type(row.get("evidence_type"))
                ...
                evidence.append(
                    StructureEvidence(
                        ...
                        ltr_structural_type=row.get("ltr_structural_type") or "none",
                        ltr_score=float(row.get("ltr_score") or 0.0),
                        ltr_confidence=row.get("ltr_confidence") or "LOW",
                        tg_ca_motif=parse_bool(row.get("tg_ca_motif", False)),
                        ppt_like=parse_bool(row.get("ppt_like", False)),
                    )
                )
        ...
        for row in reader:
            ...
            if parse_bool(row.get("ltr_present", False)):
                evidence.append(
                    StructureEvidence(
                        family_id=family_id,
                        evidence_type="LTR",
                        score=float(row.get("ltr_score") or 1.0),
                        source="structure_summary:LTR",
                        structural_type=row.get("structural_type") or "unknown",
                        ltr_structural_type=row.get("ltr_structural_type") or "none",
                        ltr_score=float(row.get("ltr_score") or 0.0),
                        ltr_confidence=row.get("ltr_confidence") or "LOW",
